...
class Moves:
    ...
    def blocked(self, new_pos, index):
        ...
        if abs(index[0]) == abs(index[1]):
            ...
            # appending data
            for x in range(abs(index[0])):
                data_x.append((eval('{} {} {}'.format(self.target_pos[0], signe_x, (100 * (x + 1))))))

            for y in range(abs(index[1])):
                data_y.append((eval('{} {} {}'.format(self.target_pos[1], signe_y, (72 * (y + 1))))))

            for i in range(len(data_x)):
                path_xy.append((data_x[i], data_y[i]))

            for i in range(len(path_xy)):
                for k in range(len(self.data)):
                    if path_xy[i] == self.data[k]['pos']:
                        if self.match:
                            blocked_xy = False
                        else:
                            blocked_xy = True
    ...
    def round(self, target_pos, new_pos, color):
        ...
        if ((target_pos[0] == new_pos[0] and (target_pos[1] // 72) in range(8))
            or target_pos[0] // 100 in range(9) and target_pos[1] == new_pos[1]) \
                and self.suicide is False and blocked[1] is False:
            return True
    ...
    def queen(self, target_pos, new_pos, color):
        ...
        elif ((target_pos[0] == new_pos[0] and (target_pos[1] // 72) in range(8))
              or target_pos[0] // 100 in range(9) and target_pos[1] == new_pos[1]) \
                and self.suicide is False and block[1] is False:
            return True
